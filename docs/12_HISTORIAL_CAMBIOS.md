...
- 2026-08-13: Convertido el panel de miembro en un cuadro de mando, revisando la entrada anterior de hoy: las tarjetas ya no son bloques plegables dentro de la pestana Perfil, sino una portada (`#inicio`) de tarjetas con nombre, imagen y descripcion breve que abren cada pantalla. Retirada la barra horizontal de pestanas, redundante con la portada y con la navegacion lateral; en su lugar hay un enlace "Volver al panel" que solo aparece fuera de la portada. Cada seccion del curriculum (Formacion, Experiencia profesional y la personalizada) pasa a ser una pantalla propia con ancla legible (`#formacion`, `#experiencia`, `#seccion-personalizada`), cabecera, cuatro indicadores calculados sobre sus articulos (entradas, activas en el PDF, con imagen y periodo, con `cv_section_metrics()` en PHP y el mismo calculo en JavaScript para no recargar al crear, editar o borrar) y el listado con Ver / Editar / Borrar y panel lateral que ya existia. El formulario de perfil envuelve ahora esas cuatro pantallas, asi que se muestra con su barra de guardado solo cuando la pantalla activa es una de ellas y su `action` apunta a la seccion en la que esta el miembro, para volver a ella al guardar. `activateMemberPanel()` pasa de estilos en linea a la clase `active` que ya define el CSS, y el listener de `invalid` salta a la pantalla del campo obligatorio vacio en lugar de desplegar una tarjeta. Las academias no ven "Pagina web" (ni tarjeta, ni enlace lateral, ni seccion, ni handler de POST): su web publica es la microficha `/academia/{slug}` que sirve `academia.php`, y el constructor de pagina de un solo scroll no se usa ahi. Anadido el panel oscuro propio a la cabecera de cada pantalla porque el fondo de esa zona pasa de negro a crema segun el scroll y el texto claro quedaba invisible sobre la parte baja. Corregido de paso un fallo preexistente: `.profile-avatar img` fuerza `display: block`, que gana al atributo `hidden` del `<img>` de reserva, asi que todo miembro sin fotografia principal veia el texto alternativo ("Fotografia principal de X") encima de su inicial en la barra lateral.
- 2026-08-13: Rehecho el bloque "Ajustes de la seccion" del panel de miembro, que seguia siendo la fila de controles apretados contra el margen derecho que heredaba de la cabecera antigua. Ahora es un panel plegable con chevron propio, resumen ("Se imprime" / "No se imprime") que sigue al interruptor aunque este cerrado, un control por celda y una linea de ayuda bajo cada uno explicando que hace. "Activa en PDF" pasa de casilla a interruptor accesible: la casilla real sigue siendo la que se envia y la que recibe el foco, solo queda oculta visualmente. El marcado propio (`.member-settings`) sustituye a `.cv-section-tools` y `.cv-section-toggle`, que ya no se usan en ningun sitio. Corregido de paso el contraste de `.button-secondary` en dos superficies claras que la regla general no cubria: el boton "Crear ..." del estado vacio y los botones "Cerrar" y "Editar" del pie del panel lateral quedaban en blanco sobre blanco, porque el arreglo de contraste excluye `.member-panel-section` (alli la seccion es transparente) y estos dos bloques si tienen fondo claro propio.
- 2026-08-21: Preparado el primer slide de la portada para el banner "Tu web, tu arte, tu legado". Como la pieza ya trae dentro su titular, su texto y sus cuatro iconos, el slide va a sangre (`.story-slide-full`): sin la columna de copy que usan los otros dos, que duplicaria el mensaje y dejaria el titular de la imagen diminuto. Se usa `object-fit: contain` y no `cover` porque el banner es de 2.5:1 y la caja del slider llega a 3.5:1, de modo que recortar le comeria el titular por arriba y los iconos por abajo; las bandas laterales que deja caen sobre fondo oscuro y no se aprecian. La imagen se carga con `loading="eager"` y `fetchpriority="high"` por estar sobre la linea de flotacion. `slider/slider01.php` comprueba con `is_file()` que la imagen exista y, si no, sigue pintando el slide anterior de Comunidad flamenca, para que la portada nunca salga con una imagen rota; el fichero pendiente es `assets/images/slider/tu-web-tu-arte-tu-legado.webp`. No se toca `comunidad-flamenca-esquema.svg`, que se sigue usando en la tarjeta "Mi academia" del panel de miembro.
- 2026-08-21: Ajustado el primer slide de la portada a la ilustracion definitiva, corrigiendo la entrada anterior de hoy: no se usa el banner completo con el texto incrustado, sino solo el artwork recortado (`assets/images/slider/slider01.png`, 1017x787), colocado en el hueco de imagen con la columna de texto al lado como los otros dos slides. Retirado por tanto el slide a sangre (`.story-slide-full`) y el `is_file()` de reserva que lo acompanaba, que ya no tenian uso. Anadida la variante `.story-slide-media-artwork`: el `.story-slide-media img` por defecto usa `object-fit: cover`, pensado para imagenes de relleno, y con un artwork mas alto (1.29:1) que el hueco (en torno a 1.6:1) le recortaba la mano levantada del bailaor por arriba y los pies por abajo; con `contain` se ve entero y las bandas laterales caen sobre fondo oscuro. Corregidos tambien el `alt`, que seguia describiendo el esquema vectorial anterior, y los atributos `width`/`height`, que declaraban 960x720 y no las medidas reales del fichero. La imagen pasa a `loading="eager"` con `fetchpriority="high"` por estar sobre la linea de flotacion y ser la candidata a LCP de la portada.
- 2026-08-23: Nuevo apartado "Inicio" en la microweb publica del artista y en su panel. Es una presentacion por articulos (ano, titulo, descripcion con el editor enriquecido que ya usaba el curriculum, imagen opcional, orden y visible) que abre la pagina publica justo debajo del hero y se gestiona desde una pantalla propia del panel. No hay tabla nueva ni migracion: los articulos viven en `perfil_json` como `intro_articles`, al lado de `education` o `experience`, asi que basta con anadir la clave en `default_member_profile()`. El CRUD reutiliza tal cual el listado con panel lateral de las secciones del curriculum (`cv_entry_item_markup()`, `cv_entry_fields_markup()`, `clean_cv_entries()`, la subida AJAX `upload_cv_entry_image` y la barra de guardado), generalizando solo lo justo: que campo hace de titulo y cuales de resumen (`summary_title`, `summary_meta`), la etiqueta del interruptor de visibilidad y el tipo de cada campo, mas un boton Ocultar/Mostrar en la fila que activan las secciones que lo declaran. En la web, los articulos se apilan en la misma celda de una rejilla y solo uno esta visible, de modo que el bloque mide lo que el mas largo y el fundido no mueve nada; con un solo articulo no se pintan controles. En movil solo ocupa sitio el visible, porque mantener el alto del mas largo dejaba un hueco enorme bajo los cortos. Resuelta de paso una colision de anclas: el hero ya usaba `id="inicio"`, asi que pasa a `#portada` (con el logo y el enlace del pie apuntando ahi) y `#inicio` queda para el bloque nuevo, que es lo que abre el menu. Acotadas al panel las reglas de impresion `#inicio`, `#pagina-web`, `#tarjeta-miembro`, `#banners` y `#seguridad` de `styles.css`, porque el micrositio carga esa misma hoja y `#inicio` habria desaparecido al imprimir. Las academias no ven el apartado: su web publica es `/academia/{slug}`, igual que ya ocurre con "Pagina web". Seguridad: el perfil se lee siempre de la sesion y los articulos no llevan id ni propietario en el formulario (solo indices posicionales que se reconstruyen enteros en cada guardado), asi que no hay identificador que manipular para tocar el contenido de otro miembro; el POST sigue pasando por `verify_csrf()`.
- 2026-08-23: Rediseñado el panel de miembro como cuadro de mando sin barra lateral. `panel-usuario.php` pierde el `aside` (`.member-sidebar`), que repetia la navegacion de la portada de tarjetas; sus datos se reparten: numero de miembro, tipo de membresia y porcentaje de perfil pasan a una linea de meta bajo el nombre en la cabecera, y el boton de imprimir el curriculum a la pantalla nueva de curriculum. La portada se organiza en tres bloques —herramientas principales (Mi perfil, Mi curriculum, Mi pagina web, Ver mi web), "Contenido de tu web" y "Servicios y cuenta"— con un sistema de tarjetas plano y comun (`.member-tile`, tres tamanos) de icono SVG en linea, titulo, descripcion, indicador y accion, en rejilla `auto-fit` que pasa de cuatro columnas a dos y a una. Los atajos de "Contenido de tu web" no son pantallas nuevas: Galeria, Videos, Agenda, Actualidad y Contacto abren "Mi pagina web" y desplazan hasta su bloque, que ahora tiene ancla propia (`#web-galeria` y companeros), porque esos bloques comparten un unico formulario que se guarda de una vez y separarlos habria roto el guardado. Separados curriculum y pagina web como conceptos: nueva pantalla `#curriculum` que agrupa las secciones del curriculum, se lleva los ajustes del PDF (fondo de cabecera y datos profesionales, que estaban sueltos en el formulario de perfil) y ofrece la descarga. "Mi perfil" deja de ser un formulario gigante y pasa a bloques de resumen (identidad, ubicacion y contacto, trayectoria, imagen) que despliegan bajo demanda exactamente el mismo formulario de antes, sin tocar ningun campo. Anadida "Premios y reconocimientos" como cuarta seccion del curriculum: la clave `awards` ya existia en el perfil sin pantalla que la usara, asi que solo hubo que declararla en `$cvSectionConfig`. Sin barra lateral, la orientacion la dan unas migas de pan que nombran la pantalla actual y, en las secciones del curriculum, su pantalla madre. Corregidos dos fallos de contraste detectados al revisar el render: los rotulos de grupo de la portada caian en la franja donde el fondo pasa de negro a crema y quedaban ilegibles (ahora van en pastilla oscura propia), y el boton "Editar" de los bloques de perfil salia blanco sobre crema, por la misma exclusion de `.member-panel-section` de la regla de contraste de `.button-secondary`. Sin cambios en base de datos.
- 2026-08-23: "Mi pagina web" adopta la misma filosofia que "Mi curriculum": `#pagina-web` deja de ser un formulario largo con siete bloques encadenados y pasa a ser un hub de tarjetas con contador, y cada bloque (slider de cabecera, galeria, videos, agenda, actualidad, redes sociales y contacto) tiene su propia pantalla. La reorganizacion no toca el guardado: el formulario `update_web_page` sigue siendo uno solo y ahora envuelve las siete pantallas, igual que el de perfil envuelve las suyas, asi que ni el handler ni los nombres de los campos ni las subidas cambian. `activateMemberPanel()` se generaliza para mostrar el formulario que contiene la pantalla activa (`form[data-panel-form]`) en lugar de conocer solo el de perfil. Las migas de pan ganan el nivel intermedio "Mi pagina web", clicable, para volver al hub. El bloque "Contenido de tu web" de la portada y las tarjetas del hub se generan de la misma lista (`$webSectionCards`), de modo que los atajos llevan ya a la pantalla concreta y desaparece el desplazamiento por anclas que se uso antes. Los indicadores de todas las pantallas pasan por delante del titular con `order: -1`, sin mover marcado, y reciben un tinte suave distinto por posicion (burdeos, dorado, azul y verde) para distinguirlos de un vistazo. Comprobado que no hay contenido de demostracion: un perfil nuevo nace con galeria, videos, eventos, actualidad, redes, articulos de inicio y premios vacios; lo unico "predefinido" son los tres huecos de slide que reserva `default_member_web_page()`, que ya no cuentan como cabeceras creadas en los contadores hasta que tienen imagen o texto.

### 2026-08-31 - Fase 1 red social - Perfil de artista, eventos, agenda y puntos

- Inicio de la evolucion hacia una red social especializada en flamenco, por secciones y sin romper nada de lo existente.
- Creada la migracion incremental `database/20260831_fase1_red_social.sql`, idempotente y reejecutable.
- Anadidas las tablas `municipios`, `eventos`, `puntos_saldos`, `puntos_movimientos`, `miembro_redes` y `registro_actividad`.
- Reutilizadas las tablas que ya existian: `provincias` (ahora sembrada con las 52), `disciplinas` y `miembro_disciplinas` (Baile, Cante, Toque y Percusion en relacion N:M) y `pagos_stripe` para la compra de puntos.
- Anadidas a `miembros` las columnas `provincia_id` y `municipio_id`, ambas NULL: ninguna ficha existente queda invalida.
- Creados los modulos `app/geo_repository.php`, `app/points_repository.php`, `app/events_repository.php`, `app/social_links_repository.php`, `app/activity_log.php`, `app/events_ui.php` y `app/points_ui.php`.
- Creadas las paginas publicas `agenda.php` (`/agenda`) y `evento.php` (`/evento/{slug}`), con sus reglas de reescritura.
- Sistema de puntos con libro mayor: `puntos_movimientos` solo se anade y `puntos_saldos` se actualiza siempre en la misma transaccion, con bloqueo de fila. El saldo negativo es imposible por comprobacion, por columna UNSIGNED y por serializacion.
- Los costes viven exclusivamente en el servidor (`csf_puntos_costes()`): promocionar un evento cuesta 10 puntos y un enlace social adicional 2. Lo que envie el formulario no interviene en el cobro.
- Crear y editar eventos es gratis siempre, por decision estrategica: la agenda necesita contenido antes que ingresos.
- Puntos iniciales de bienvenida: 30 para miembro gratuito y 100 para VIP, abonados de forma idempotente al entrar por primera vez en el panel.
- Interfaz de compra de puntos terminada con sus cinco paquetes. No acredita puntos: registra la intencion en `pagos_stripe` y avisa de que el pago con Stripe llegara despues.
- Redes sociales: mostrarlas es gratis, el primer enlace clicable tambien, y los siguientes cuestan 2 puntos. Una activacion pagada no se pierde ni se reembolsa nunca.
- Panel del miembro: nuevas tarjetas Mis eventos, Mis puntos y Redes sociales, cabecera resumen con saludo, saldo, proximos eventos y porcentaje de perfil, y CTA principal de crear evento.
- Perfil del artista: provincia pasa a lista normalizada conservando lo ya escrito, municipio con autocompletado y seleccion multiple de disciplinas.
- Perfil publico del artista: bloques de proximos eventos, eventos anteriores y redes sociales desde base de datos.
- Directorio de artistas y agenda con filtros por provincia, municipio y disciplina.
- Panel de administracion: secciones Eventos y Puntos, de solo lectura, con metricas y auditoria.
- Ocultadas de la portada del panel, sin borrar, las dos tarjetas sustituidas: Agenda (`web-eventos`) y Redes sociales (`web-redes`). Siguen accesibles y editables desde el hub de Mi pagina web, marcadas como version anterior, y se recuperan poniendo `$ocultarTarjetasSustituidas` a false.
- La agenda antigua guardada en `perfil_json` se sigue mostrando en el perfil publico de los miembros que aun no han publicado eventos en la tabla nueva.
